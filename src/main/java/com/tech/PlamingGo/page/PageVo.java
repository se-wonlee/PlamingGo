...
public class PageVo {
	...
	public void pageCalculate(Integer totDrama) {
		getPage();									// Getter 에서 현재페이지 가져오기
		totRow = totDrama;								// 받아온 total을 totRow로 대입 → 전체 데이터 수
		totPage = (int) (totDrama / displayRowCount);	// 전체 페이지 수 = 전체 데이터 수 / 한 페이지에 출력할 데이터 개수 의 몫
		
		if (totDrama % displayRowCount > 0) {			// 나머지 > 0 = 딱 나누어져서 떨어지지 않으면 (% : 나머지 연산자)
