...
public class CPageVO {
    private Integer displayRowCount = 9;           // 출력할 데이터 개수(1페이지에 3개)
     ...
     */
    public void pageCalculate(Integer total) { //Integer total를 가지고있는 pageCalculater가 호출되면 페이지수계산
    	int pageGroupCnt=9;
        ...
        if ( total % displayRowCount > 0 ) { //토탈페이지를 3으로나눠서 0보다 작으면 페이지하나를 추가
            totPage++;
        }

        pageStart = (page - (page - 1) % pageGroupCnt) ; //스타트페이지
        pageEnd = pageStart + 2;
        if (pageEnd > totPage) {
            pageEnd = totPage;
        }
