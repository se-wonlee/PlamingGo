...
<head>
	...
	<style type="text/css">
		#intro {
		  width: 100%;
		  height: 100vh;
		  background: url("resources/upload-drama/${detailview.img }") top center;
...
<body>
	...
	<section id="intro">
		<div class="intro-container wow fadeIn">
			<h1 class="mb-4 pb-0">지금 바로,<br><span>&nbsp;${detailview.title }</span>의<br />예고편을 감상하세요!</h1>
			<%-- <p class="mb-4 pb-0">${detailview.title }<br />지금 바로 감상하세요!</p> --%>
			<a href="${detailview.trailer }" class="venobox play-btn mb-4" data-vbtype="video"
	...
	<!-- 관리자 : 수정 / 삭제 버튼 -->
		<c:if test="${code==0 }">
			&nbsp;&nbsp;
			<!-- 수정 버튼 -->
			<a class="btn btn-outline-danger" href="drama/modify?c=${detailview.code }" role="button">수정</a>
				  ...
				  <div class="modal-dialog">
				    <div class="modal-content">
				      ...
				      <div class="modal-footer">
				        <a class="btn btn-outline-danger" href="drama/delete?c=${detailview.code } " role="button">확인</a>
	...
	<!-- 관리자 -->	
		...
		<!-- 헤더  -->
		<h1 style="font-family:'Noto Sans KR', sans-serif; font-size: 35px;" id="section1">
			&nbsp;&nbsp;&nbsp;기본 정보
		</h1>
		<hr /> <!-- 줄 -->
		...
		<section id="speakers-details" class="wow fadeIn">
			<div class="container">
				...
				<div class="row">
			    	<!-- 왼쪽 : 포스터 이미지 -->
			    	<div class="col-md-6">
			    		<!-- 드라마 상세보기에서 왼쪽 포스터 이미지를 클릭하면 에피소드 section으로 보내줌  -->
				    	<img src="resources/upload-drama/${detailview.img }" class="img-fluid"
			        ...
			        <div class="col-md-6">
			            <div class="details">
			            	<!-- 제목(TITLE)/별점(RATING) -->
			            	<h2>${detailview.title } <span class="badge badge-pill badge-dark"><i class="fa fa-star"></i> ${detailview.rating }</span></h2>
			              	
			              	<dl class="row"> 	
								<dt class="col-sm-3">제작년도</dt>
							  	<dd class="col-sm-9">${detailview.opendate }</dd>
							  
								<dt class="col-sm-3">국가</dt>
  								<dd class="col-sm-9">${detailview.nation }</dd>
  								
  								<dt class="col-sm-3">개요</dt>
  								<dd class="col-sm-9">${detailview.age }&nbsp;/&nbsp;에피소드 ${detailview.epi }개</dd>
  								
  								<dt class="col-sm-3">감독</dt>
  								<dd class="col-sm-9">${detailview.director }</dd>
  								
  								<dt class="col-sm-3">배우</dt>
  								<dd class="col-sm-9">${detailview.actor }</dd>
							
  								<dt class="col-sm-3">장르</dt>
  								<dd class="col-sm-9">${detailview.genre }</dd>
								
								<dt class="col-sm-3">줄거리</dt>
  								<dd class="col-sm-9">${detailview.summary }</dd>
							</dl>
			            </div> <!-- 드라마 정보 끝  -->
			        </div> <!-- 오른쪽 grid 끝 --> 
			...
			<!-- 조회수 증가 -->
				 ...
				 &nbsp;현재 <strong>${detailview.hits }</strong>명의 플라밍고 회원님이 <strong>${detailview.title }</strong>에 관심을 보였어요!
				 <hr />
				 <br />
			</div>	<!-- 조회수까지 컨테이너 끝  -->	 
    	 </section>  
      ...
      ===============================-->
		...
		<!-- 댓글 섹션 시작 -->
      	<section id="contact" class="section-bg ">
      		...
      		<span style="font-family:'Noto Sans KR', sans-serif; font-size: 20px;">&nbsp;&nbsp;&nbsp;댓글</span>
      		<br /><br />
			...
			<table class="table table-hover">
		  		<tbody>
					...
					<tr>
				    	<td><span style="color: #593996;"><i class="fa fa-user"></i></span>&nbsp;&nbsp;&nbsp;${review.writer }</td>
					    ...
					    <td></td>
					    <td align="right">
					    	<f:formatDate value="${review.regdate }" pattern="yyyy-MM-dd"/>
					    </td>
					    <td colspan="2" align="right">
					    	<c:if test="${id eq review.writer || code==0 }">
					    		<%-- <a href="dreview_modify?rno=${review.rno }&bno=${reveiew.bno }">수정</a> &nbsp; --%>
					    		<a href="dreview_delete?rno=${review.rno }&bno=${detailview.code }"><i class="fa fa-times"></i></a>
					    	</c:if>
					    </td>
					    <td></td>
				    </tr>
			        	...
			        	<div class="form-row">
			     		...
			     		<div class="form-group">
			     			&nbsp;<span style="color: #593996;"><i class="fa fa-edit"></i></span>&nbsp; <span id="reviewReply">감상평</span>
			              	<textarea class="form-control" name="content" rows="5" data-rule="required" data-msg="내용을 입력해주세요." placeholder="${id }님의  리뷰가 다른 회원에게 도움이 돼요!"></textarea>
			              	<div class="validation"></div>
			            </div>
			     		...
			     		<div class="form-row">
			            	<div class="form-group col-md-3">
			                	...
			                	<input type="date" class="form-control" name="regdate" id="datePicker" value="yyyy-MM-dd" disabled/>
			                	<div class="validation"></div>
			              	</div>
			            </div>
			            <br />
